...
extends GuiScreen {
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
		...
		if (MathUtils.isMouseHovered(width / 2 - 100, l1 + 158, 200, 25, mouseX, mouseY) && mouseButton == 0) {
			this.mc.getSoundHandler().playSound(PositionedSoundRecord.create(new ResourceLocation("gui.button.press"), 1.0F));
			this.mc.shutdown();
		}
		if (MathUtils.isMouseHovered(width / 2 - 100, l1 + 128, 200, 25, mouseX, mouseY) && mouseButton == 0) {
			login();
		}
	}
