...
public final class GuiAddAccount
    ...
    public GuiAddAccount(GuiAltManager previousScreen) {
    ...
    protected void actionPerformed(GuiButton button) {
        switch (button.id) {
            ...
            case 0: {
                ...
                AtomicReference<Alt> alt = new AtomicReference<>();
                ...
                new Thread(() -> {
                    try {
                        ...
                        if (loginModeName == LoginMode.MICROSOFT && password.getText().isEmpty()) {
                            alt.set(LoginManager.crackedLogin(username.getText()));
                        } else {
                            switch (loginModeName) {
                                case RNG:
                                    alt.set(LoginManager.crackedLogin(finalName));
                                    break;
                                case MICROSOFT:
                                    alt.set(LoginManager.microsoftEmailLogin(username.getText(), password.getText()));
                                    break;
                                case MICROSOFT_BROWSER:
                                    alt.set(LoginManager.microsoftBrowserLogin());
                                    break;
                            }
                        }
                        status = EnumChatFormatting.GREEN + "Logged in as " + alt.get().getUsername() + "!";

                        AltHandler.addAlt(alt.get());

                        this.previousScreen.refreshAccountList();
    ...
    public void initGui() {
        ...
        this.buttonList.add(new GuiButton(0, width / 2, var3 + 72 + 12, 100, 20, "Add"));
