...
public class IntellijCategory {
    ...
    public void drawScreen(int mouseX, int mouseY, int offset) {
        ...
        Menace.instance.jetbrainsMono.drawString(this.category.getCategoryName().toLowerCase(), this.x + 35, this.y + 20 + offset, new Color(170, 177, 181, 255).getRGB());
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton, int offset) {
        if (MathUtils.isMouseHovered(this.x + 10, (int) (this.y + 20 + offset), Menace.instance.jetbrainsMono.getStringWidth(this.category.getCategoryName().toLowerCase()) + 17, Menace.instance.jetbrainsMono.getHeight(), mouseX, mouseY)) {
            dropdown = !dropdown;
        }
