...
public class SettingsMenuComponent {
    ...
    private int scroll;
    private final Module component;
    private ArrayList<SettingComponent> settingComponents = new ArrayList<>();

    public SettingsMenuComponent(MenaceClickGui parent, Module component) {
        this.x = parent.x;
        this.y = parent.y;
        ...
        for (Setting s : component.getSettings()) {
            if (s instanceof BooleanSetting) {
                settingComponents.add(new ToggleComponent((BooleanSetting) s));
            } else if (s instanceof NumberSetting) {
                settingComponents.add(new SliderComponent((NumberSetting) s));
    ...
    public void draw(int mouseX, int mouseY) {
        ...
        //Module description
        fontRenderer.drawCenteredString(component.getDescription(), (float) (this.x + 200 + (this.width - 200)) / 2, this.y + 25, Color.WHITE.getRGB());
