...
public class CategorySelectorComponent {
    ...
    public void draw(int mouseX, int mouseY) {
        //Category bar
        if (MathUtils.isMouseHovered2(this.categoryX, this.categoryY, this.categoryWidth, this.y + 65, mouseX, mouseY) && categoryBarExpanded && categoryAntimationFinished) {
            ...
            for (Category category : Category.values()) {
                ...
                ResourceLocation icon = new ResourceLocation("menace/menaceui/" + category.getCategoryName().toLowerCase() + ".png");
                RenderUtils.drawImage(this.categoryX + 7 + xOff, this.categoryY + 7, 32, 32, icon, new Color(255, 255, 255, 0));
                fontRenderer.drawCenteredString(category.getCategoryName(), this.categoryX + 23 + xOff, this.categoryY + 50, Color.WHITE.getRGB());

                xOff += 60;
            }

        } else if (MathUtils.isMouseHovered2(this.categoryX, this.categoryY, this.categoryWidth, (int) (this.y + Math.max(15, categoryBarAnimation.getValue())), mouseX, mouseY) && categoryBarExpanded && !categoryAntimationFinished) {
            categoryCloseAnimation = false;
        ...
        } else if (MathUtils.isMouseHovered2(this.categoryX, this.categoryY, this.categoryWidth, this.y + 15, mouseX, mouseY) && !categoryBarExpanded) {
    ...
    public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
        if (MathUtils.isMouseHovered2(this.categoryX, this.categoryY, this.categoryWidth, this.y + 65, mouseX, mouseY) && categoryBarExpanded && categoryAntimationFinished) {
            int xOff = 10;
            for (Category category : Category.values()) {
                if (MathUtils.isMouseHovered2(this.categoryX + 5 + xOff, this.categoryY + 6, this.categoryX + 5 + xOff + 35, this.categoryY + 40, mouseX, mouseY)) {
