...
public class DropdownCategory {
    ...
    public DropdownCategory(Category category, int x, int y) {
        ...
        for (Module module : Menace.instance.moduleManager.getModulesByCategory(category)) {
    ...
    public void draw(int mouseX, int mouseY) {
        ...
        fr.drawCenteredString(category.getCategoryName(), x + width / 2, y + height / 2, Color.WHITE.getRGB());
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (MathUtils.isMouseHovered(x, y, width, height, mouseX, mouseY) && mouseButton == 1) {
            expanded = !expanded;
        }
    ...
    public void handleScroll(int mouseX, int mouseY, int dWheel) {
        if (MathUtils.isMouseHovered(x, y, width, height, mouseX, mouseY) && dWheel != 0) {
            scrollY += dWheel / 120;
        }
    }
